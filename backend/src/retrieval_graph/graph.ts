...
async function retrieveDocuments(
  ...
  const response = await retriever.invoke(state.query, { configurable: { filter: { docId: config.configurable?.docId } } });
  console.log(`Retrieved ${response.length} documents for docId: ${config.configurable?.docId}`);
