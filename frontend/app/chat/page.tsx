...
function ChatPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch(
        `/api/corporate-actions/by-id?id=${encodeURIComponent(docId)}`,
        { cache: "no-store" }
      );
      if (!res.ok) {
        setAction(null);
  ...
  return (
    <main 
    key={docId || 'default'} 
    className="flex min-h-screen flex-col items-center p-4 md:p-24 max-w-5xl mx-auto w-full pb-32">
      
      {/* Corporate Action Details */}
      {action && (
        <div className="mb-4 p-4 rounded bg-gray-50 w-full">
          <div className="space-y-1">
            <div className="text-xs text-gray-500">Company</div>
            <div className="text-lg font-semibold">{action.company}</div>
            ...
            <div className="text-sm">{action.subject}</div>

            <div className="text-xs text-gray-500 mt-2">Announcement Date</div>
            <div className="text-sm">
              {action.announcement_datetime
                ? new Date(action.announcement_datetime).toLocaleString()
                : "—"}
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <p className="font-medium text-muted-foreground max-w-md mx-auto">
                This ai chatbot can give you insights on announcements:{' '}
                <a
                  href="https://www.bseindia.com/corporates/ann.html"
                  className="underline hover:text-foreground"
                >
                  made on BSE or NSE for corporate actions for the Listed Companies
